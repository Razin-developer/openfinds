...
fun DevicesScreen(
    ...
    Scaffold(
            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = onManageGroups) {
                        Icon(Icons.Outlined.Groups, contentDescription = "Manage groups")
                    }
                    IconButton(onClick = { sortMenuOpen = true }) {
                        Icon(Icons.Outlined.Sort, contentDescription = "Sort")
                    }
                    DropdownMenu(expanded = sortMenuOpen, onDismissRequest = { sortMenuOpen = false }) {
                        DropdownMenuItem(text = { Text("Online first") }, onClick = {
                            viewModel.onSortOrderChange(DeviceSortOrder.ONLINE_FIRST)
                            sortMenuOpen = false
                        })
                        DropdownMenuItem(text = { Text("Name") }, onClick = {
                            viewModel.onSortOrderChange(DeviceSortOrder.NAME)
                            sortMenuOpen = false
                        })
                        DropdownMenuItem(text = { Text("Last seen") }, onClick = {
                            viewModel.onSortOrderChange(DeviceSortOrder.LAST_SEEN)
                            sortMenuOpen = false
                        })
                    }
                },
        ...
        floatingActionButton = {
            ExtendedFloatingActionButton(onClick = onAddDevice, icon = {
                ...
                Row(
                    ...
                    uiState.groups.forEach { group ->
                        FilterChip(
                            ...
                            onClick = { viewModel.onGroupFilterChange(group.id) },
