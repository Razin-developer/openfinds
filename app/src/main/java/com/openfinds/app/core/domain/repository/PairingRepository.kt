...
interface PairingRepository {
    ...
    suspend fun pairViaQr(
        host: String,
        port: Int,
        expectedIdentityPublicKeyB64: String,
    ): PairingOutcome

    suspend fun pairViaPin(
        host: String,
        port: Int,
        pin: String,
    ): PairingOutcome
...
class PairingRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun pairViaQr(
            host: String,
            port: Int,
            expectedIdentityPublicKeyB64: String,
        ): PairingOutcome =
        ...
        override suspend fun pairViaPin(
            host: String,
            port: Int,
            pin: String,
        ): PairingOutcome = persistIfSuccessful(connectionManager.pairWith(host, port, PairingMode.PIN, pin = pin))
        ...
        override suspend fun acceptIncoming(request: PairingRequest) {
            ...
            historyRepository.record(request.fromDeviceId, request.fromDeviceName, HistoryEventType.PAIRED)
        }
        ...
        private suspend fun persistIfSuccessful(outcome: PairingOutcome): PairingOutcome {
            if (outcome is PairingOutcome.Success) {
                ...
                historyRepository.record(outcome.peerDeviceId, outcome.peerDeviceName, HistoryEventType.PAIRED)
            }
            return outcome
        }
