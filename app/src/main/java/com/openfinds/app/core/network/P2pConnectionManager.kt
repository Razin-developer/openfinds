 ...
 *
 * Built on Ktor Network (`io.ktor.network.sockets`) rather than raw
 * `java.net` sockets, so every read/write is a suspend function that composes
 * naturally with the rest of the coroutine-based codebase.
 */
@Singleton
class P2pConnectionManager
    ...
    constructor(
        ...
        fun start() {
            ...
            scope.launch {
                runCatching {
                    aSocket(selectorManager).tcp().bind(InetSocketAddress("0.0.0.0", NetworkConstants.TCP_PORT))
                }.onSuccess { server ->
                    serverSocket = server
                    _isListening.value = true
                    Timber.i("OpenFind P2P listening on port ${NetworkConstants.TCP_PORT}")
                    acceptLoop(server)
                }.onFailure {
        ...
        private suspend fun handleIncoming(socket: io.ktor.network.sockets.Socket) {
        ...
        private suspend fun handleCommand(
            ...
            when (action) {
                DeviceAction.RING -> {
                    findActionExecutor.startRing()
                    alertsNotifier.notifyBeingFound(session.peerDeviceName)
                    session.send(P2pMessage.Ack(action))
                }
                DeviceAction.VIBRATE -> {
                    findActionExecutor.startVibrate()
                    alertsNotifier.notifyBeingFound(session.peerDeviceName)
                    session.send(P2pMessage.Ack(action))
                }
                DeviceAction.FLASH -> {
                    findActionExecutor.startFlash()
                    alertsNotifier.notifyBeingFound(session.peerDeviceName)
                    session.send(P2pMessage.Ack(action))
                }
                DeviceAction.STOP_FIND -> {
                    findActionExecutor.stopAll()
                    session.send(P2pMessage.Ack(action))
                }
